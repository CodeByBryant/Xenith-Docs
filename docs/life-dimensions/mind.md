---
sidebar_position: 3
title: Mind
description: Daily gratitude and structured thought audits for mental clarity, in Xenith's Mind dimension.
keywords:
  - gratitude journal
  - thought audit
  - CBT thought record
  - mental clarity app
  - mental wellness tracker
---

# Mind

The **Mind** dimension (shown in violet) holds Xenith's tools for mental clarity:
a daily gratitude practice and a structured thought audit. Both are quick,
private, and grounded in well-supported techniques.

## Tools

### Daily Gratitude
Log a few specific things you're grateful for each day. Specificity matters more
than quantity — *"the call with my sister"* lands deeper than *"my family."*
Gratitude practice is one of the most consistently supported habits for reducing
anxiety and improving sleep.

### Thought Audit
When an unhelpful thought is weighing on you, the Thought Audit walks you through
a structured reframe inspired by cognitive behavioral therapy (CBT) thought
records:

1. **Record** the thought.
2. **Examine the evidence** for and against it.
3. **Name the distortion** — catastrophizing, all-or-nothing thinking,
   mind-reading, and so on.
4. **Reframe** it into a more balanced alternative.
5. **Rate the shift** in how strongly you believe the original thought.

Tracking your most common distortion over time builds genuine self-awareness.

> **Not a substitute for therapy**
>
> Xenith's Mind tools are self-guided wellness practices for everyday mental
> hygiene — not clinical treatment. If you're dealing with significant anxiety,
> depression, or trauma, please reach out to a qualified mental health
> professional.

## Privacy

Because this content is deeply personal, Xenith treats it with the strictest
handling: your entries are private, and are never analyzed for content or used to
train AI. See [Privacy & Data](../account/privacy-and-data.md).

## Related

- [Reflection](../daily-workflow/reflection.md) — your end-of-day review.
- [Life Dimensions overview](./overview.md)
